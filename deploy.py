...
def upload_to_dropbox(target_file_name, source_file, dropbox_token, dropbox_folder):
    ...
    if r.status_code != requests.codes.ok:
        print("Failed: upload file to Dropbox: {errcode}".format(errcode=r.json()))
        return None
...
def get_app(release_dir):
    ...
    app_version = "["+str(json_data['elements'][0]['versionCode'])+"-"+str(json_data['elements'][0]['versionName'])+"]"
